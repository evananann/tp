...
Hi there! If this is your first time using an app where you type commands instead of clicking buttons, you are in exactly the right place. This guide will walk you through everything from scratch — no prior experience needed.

**AddressBook** is a desktop app for managing contacts. You type short text commands to add, search, and organise people in your phone book. Once you get the hang of it, typing is actually much faster than hunting around with a mouse.
...
💡 <strong>Navigation tip:</strong> See the <strong>"On this page" sidebar</strong> on the left — it lists every section of this guide. Click any item to jump straight there. The sidebar stays visible as you scroll, so you can always find your way around. On a narrow screen (such as a phone), the sidebar appears at the top of the page instead.
</div>
...
This guide is written for **first-year Computer Science students** who may have never opened a terminal (the text-based window where you type computer commands — more on that below) before.

All you need to know:

- How to open files and folders on your computer.
- How to type and press Enter.
...
3. [Quick Start](#quick-start)
   ...
   - [Step 3 — Download AddressBook](#step-3--download-addressbook)
...
AddressBook is a desktop application for managing contacts quickly using typed commands.
...
If you can type quickly, AddressBook lets you manage hundreds of contacts faster than any click-based app.
...
> A text window where you communicate with your computer by typing instructions. Instead of clicking icons, you type commands like `cd Documents` or `java -jar addressbook.jar`. Think of it like sending text messages to your computer.
>
...
**File path**
> The full address of a file or folder on your computer — like a postal address, but for files.
>
> - On **Windows**, it looks like: `C:\Users\yourname\Documents\AddressBook`
> - On **macOS/Linux**, it looks like: `/Users/yourname/Documents/AddressBook`
> The `~` symbol is a shortcut that means "your home folder." So `~/Documents` means `/Users/yourname/Documents`.

**JAR file (`.jar`)**
> A special file type that packages a runnable Java application. `addressbook.jar` is the file that contains and runs AddressBook. You launch it with a terminal command rather than double-clicking it.

**Java**
> A programming language and platform that AddressBook is built on. Your computer needs Java installed for AddressBook to work — similar to how you need a PDF reader to open a PDF. We need **Java 17** specifically.
...
**Command**
> An instruction you type into AddressBook's command box. For example, `list` is a command that shows all your contacts.
...
**Prefix**
> The short code before each piece of information, like `n/` for name, `p/` for phone, or `e/` for email. Each prefix tells AddressBook what type of information follows.
...
**JSON file (`.json`)**
> A plain-text file format used to store data in an organised way. AddressBook saves all your contacts in a file called `addressbook.json`. You can open it in any text editor if you ever need to.
...
This section walks you through installing and launching AddressBook for the first time. Follow the steps for your operating system (**Windows** or **macOS**).
...
AddressBook needs **Java 17** (a specific version of the Java platform) to run. Let's check if you have it.
...
- **macOS:** Follow the [macOS Java installation guide](https://se-education.org/guides/tutorials/javaInstallationMac.html) to install the correct version. macOS requires a specific JDK variant (Azul JDK 17 with JavaFX support) for AddressBook to display correctly.

Once installed, close and reopen your terminal, then run `java -version` again to confirm.

---

### Step 3 — Download AddressBook

1. Go to the [AddressBook releases page](https://github.com/se-edu/addressbook-level3/releases) in your web browser.
2. Under the latest release, click `addressbook.jar` to download it. This is the JAR file (the runnable application file) that contains AddressBook.
3. Create a new folder somewhere easy to remember — for example:
   - **Windows:** `C:\Users\yourname\AddressBook`
   - **macOS:** `/Users/yourname/Documents/AddressBook` (you can also just call it `AddressBook` inside your Documents folder)
4. Move `addressbook.jar` into that folder.

<div markdown="block" class="alert alert-info">
💡 <strong>Why a dedicated folder?</strong> AddressBook automatically creates a <code>data</code> folder next to <code>addressbook.jar</code> to save your contacts. Keeping <code>addressbook.jar</code> in its own folder keeps everything tidy.
</div>
...
Now you need to tell your terminal to "go to" the folder where `addressbook.jar` lives. You do this with the `cd` command (short for **c**hange **d**irectory — in other words, change which folder you're in).

**On Windows:**

```
cd C:\Users\yourname\AddressBook
```
...
```
cd ~/Documents/AddressBook
```

Press **Enter**.

<div markdown="block" class="alert alert-info">
💡 <strong>Tip:</strong> If your folder path contains spaces (e.g. <code>My AddressBook</code>), wrap the whole path in quotation marks:<br>
<code>cd "C:\Users\yourname\My AddressBook"</code>
</div>

After pressing Enter, your terminal prompt will update to show the new folder. For example, on Windows you might see `C:\Users\yourname\AddressBook>`.

To confirm `addressbook.jar` is there, you can list the files in the folder:

- **Windows:** type `dir` and press **Enter**
- **macOS/Linux:** type `ls` and press **Enter**

You should see `addressbook.jar` in the list.

---

### Step 5 — Launch the app

With your terminal open and pointing to the AddressBook folder, type the following and press **Enter**:

```
java -jar addressbook.jar
```

This tells Java to run the `addressbook.jar` file.

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> The AddressBook window opens with a set of sample contacts already loaded. You should see a contact list on the right side and a text box at the top for typing commands.
</div>

<div markdown="block" class="alert alert-warning">
⚠️ <strong>Nothing happened?</strong> Check that:
<ul>
<li>You are in the correct folder (the one that contains <code>addressbook.jar</code>).</li>
...
<!-- TODO: Replace with an actual annotated screenshot of the application window. -->

The AddressBook window (the user interface, or UI — meaning all the visual elements you interact with) contains four areas:
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> The result display shows <code>New contact added: Alex Tan; ...</code> and Alex Tan appears in the contact list on the right.
</div>

Congratulations — you've just used your first AddressBook command!
...
All AddressBook commands follow the same basic structure: a **command word** (what you want to do) followed by **parameters** (the details).
...
Each prefix (`n/`, `p/`, `e/`, etc.) tells AddressBook what type of information follows it. The table below explains what is required and what rules apply:
...
<pre>add n/Priya Sharma p/87654321 e/priya@company.com a/10 Orchard Road t/colleague</pre>
This adds a contact named Priya Sharma with a phone number, email, address, and the tag "colleague."
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>New contact added: Priya Sharma; Phone: 87654321; Email: priya@company.com; Address: 10 Orchard Road; Tags: [colleague]</code>
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — update phone and email for contact 3:</strong>
<pre>edit 3 p/99887766 e/priya.new@company.com</pre>
</div>

<div markdown="block" class="alert alert-info">
📌 <strong>Example — remove all tags from contact 3:</strong>
<pre>edit 3 t/</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Edited Person: Priya Sharma; Phone: 99887766; ...</code>
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — add a remark to contact 2:</strong>
<pre>remark 2 r/Prefers contact by email only.</pre>
</div>

<div markdown="block" class="alert alert-info">
📌 <strong>Example — remove the remark from contact 2:</strong>
<pre>remark 2 r/</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Added remark to Person: ...</code> or <code>Removed remark from Person: ...</code>
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — delete contact number 4:</strong>
<pre>delete 4</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Deleted Person: ...</code>
</div>
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Listed active contacts</code> and all active contacts appear in the list.
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — find all contacts named Alex or David:</strong>
<pre>find alex david</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>2 persons listed!</code> and the matching contacts appear.
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — show all contacts tagged "colleague" or "vip":</strong>
<pre>filter t/colleague t/vip</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>N persons listed!</code> where N is the number of matching contacts.
</div>
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Sorted all persons (starred first, then by name)</code> and the contact list reorders accordingly.
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — star contact number 2:</strong>
<pre>star 2</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Starred Person: ...</code>
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — unstar contact number 2:</strong>
<pre>unstar 2</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Unstarred Person: ...</code>
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — archive contact number 5:</strong>
<pre>archive 5</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Archived Person: ...</code> and the contact no longer appears in the active list.
</div>
...
<div markdown="block" class="alert alert-info">
📌 <strong>Example — restore the first archived contact:</strong>
<pre>listarchived
unarchive 1</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> The contact disappears from the archived list and reappears when you run <code>list</code>.
</div>
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> <code>Listed archived contacts</code> and all archived contacts appear in the list.
</div>
...
- `COMMAND_WORD` must be one of AddressBook's built-in commands (e.g. `add`, `delete`, `find`).

**Steps — create a shortcut:**

1. Type `alias add` followed by your chosen shortcut word and the command it should trigger.
2. Press **Enter**.

<div markdown="block" class="alert alert-info">
📌 <strong>Example — create shortcut <code>a</code> for the <code>add</code> command:</strong>
<pre>alias add a add</pre>
After this, you can type <code>a n/Alex p/91234567 e/alex@email.com</code> instead of the full <code>add</code> command.
</div>

<div markdown="block" class="alert alert-info">
📌 <strong>Example — see all your current shortcuts:</strong>
<pre>alias list</pre>
</div>

<div markdown="block" class="alert alert-info">
📌 <strong>Example — remove the shortcut <code>a</code>:</strong>
<pre>alias remove a</pre>
</div>

<div markdown="block" class="alert alert-success">
✅ <strong>Expected result (create):</strong> <code>Alias 'a' added for command 'add'.</code>
</div>

<div markdown="block" class="alert alert-warning">
⚠️ <strong>Heads up:</strong> Shortcuts are only kept in memory while the app is running. They are cleared every time you close AddressBook. You will need to re-enter your shortcuts at the start of each session.
</div>
...
AddressBook automatically saves all your contacts to a JSON file (a plain-text data file) at `data/addressbook.json` inside the same folder as `addressbook.jar`. If you ever need to make many changes at once, you can open this file in any plain-text editor (such as Notepad on Windows or TextEdit on macOS) and edit it directly.

<div markdown="block" class="alert alert-warning">
⚠️ <strong>Warning:</strong> JSON files must follow a strict format. If you accidentally delete a comma, bracket, or quotation mark, the file becomes invalid. When AddressBook detects an invalid file, it discards all data and starts completely empty on the next launch. Always make a backup copy of <code>addressbook.json</code> (e.g. save a copy as <code>addressbook_backup.json</code>) before editing it manually.
</div>
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> A help window opens showing a link to this User Guide.
</div>

<div markdown="block" class="alert alert-info">
💡 <strong>Tip:</strong> You can also press <strong>F1</strong> on your keyboard at any time to open the help window without typing a command. (Just press the F1 key — it's usually in the top row of your keyboard.)
</div>
...
<div markdown="block" class="alert alert-warning">
⚠️ <strong>Warning:</strong> This deletes all contacts permanently and cannot be undone. Before running this command, make a backup copy of <code>data/addressbook.json</code> in case you need the data later.
</div>
...
<div markdown="block" class="alert alert-success">
✅ <strong>Expected result:</strong> The contact list becomes empty.
</div>
...
Close AddressBook. Your data is saved automatically before the app closes — you do not need to do anything extra.
...
AddressBook saves your contacts automatically after every command that changes anything. You never need to press a "Save" button.

**Where your data lives:** `data/addressbook.json` — a file inside the same folder as `addressbook.jar`.

### Transferring your data to another computer

Want to use AddressBook on a different computer and bring your contacts with you? Here's how:

1. On your current computer, find and copy the file `data/addressbook.json`.
2. On the other computer, install Java 17 and follow this Quick Start guide to download `addressbook.jar` and launch it once. This creates the `data` folder automatically.
3. Replace the `data/addressbook.json` file on the new computer with the copy you brought over.
4. Launch AddressBook on the new computer. All your contacts will appear.
...
No. AddressBook treats contacts with identical names as duplicates and will not add the second one. Use a slightly different name (like adding a middle initial) if you genuinely need two contacts with the same name.

**Q: How do I undo a command?**

AddressBook does not have an undo command. For actions that cannot be reversed — like `delete` and `clear` — make a backup copy of `data/addressbook.json` beforehand so you can restore it if needed.

**Q: I get an error saying "Unknown command." What does that mean?**

This means AddressBook did not recognise the word you typed as a valid command. Check your spelling and make sure there are no extra spaces before the command word. Refer to the [Command Summary](#command-summary) for the full list of valid commands.
